...
function viewProductsForSale(cb){
  ...
  connection.query('SELECT * FROM Products', function(err, res){
    ...
    for (var i = 0; i < res.length; i++) {
      table.push([res[i].ItemID, res[i].product_name, res[i].department_name, '$' + res[i].Price.toFixed(2), res[i].stock_quantity]);
    }
...
function addNewProduct(){
  ...
  inquirer.prompt([
    ...
    message: 'How many of this item do we have in stock currently?'
      ...
      function(err){
        if(err) throw err;
        console.log(item.product_name + ' has been successfully added to the inventory.');
